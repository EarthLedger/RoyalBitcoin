...
/* Determine system physic memory is validate for running miner threads */
bool IsValidateForMiner()
{
    // change to MB size
#define DIV (1000*1000)
    ...
    if(sysinfo(&s_info))
    {
    	LogPrintf("%s %d|sysinfo error|\n", __FILE__, __LINE__);
    	return false;
    }

    LogPrintf("%s %d|RAM total=%u|\n", __FILE__, __LINE__, s_info.totalram);

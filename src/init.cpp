...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 7333, 17333));
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    //�ڴ��С�Ƿ���������
    bool bValidMem = IsValidateForMiner();
    if(!bValidMem)
    {
        LogPrintf("%s %d|inValid Mem\n", __FILE__, __LINE__ );
    }


#ifdef ENABLE_WALLET
    // Generate coins in the background
    if (pwalletMain && bValidMem)
